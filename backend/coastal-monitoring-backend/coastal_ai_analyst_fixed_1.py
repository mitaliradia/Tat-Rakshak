...
try:
    print("1. Using external Google Earth Engine API...")
    
    # Get external GEE API key from environment
    external_gee_api_key = os.getenv('EXTERNAL_GEE_API_KEY')
    external_gee_url = os.getenv('EXTERNAL_GEE_API_URL', 'https://api.external-gee.com')
    
    if not external_gee_api_key:
        print("Warning: EXTERNAL_GEE_API_KEY not found in environment variables")
        print("Please set EXTERNAL_GEE_API_KEY in your .env file")
        external_gee_api_key = None
    
    # Initialize with external credentials if available
    if external_gee_api_key:
        # Set up external GEE authentication
        ee.Initialize(
            project='external-gee-project',
            credentials=external_gee_api_key,
            opt_url=external_gee_url
        )
        print("✅ Connected to external Google Earth Engine API")
    else:
        # Fallback to local authentication
        print("Falling back to local GEE authentication...")
    ...
    print("Or set EXTERNAL_GEE_API_KEY in your .env file")
